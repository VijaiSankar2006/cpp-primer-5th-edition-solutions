/************************************************************************************************************************************************************
 * @file Ex_10_18.cpp
 * @brief Exercise 10.18: Rewrite biggies to use partition instead of find_if. We described the partition algorithm in exercise
 *        10.13 in § 10.3.1 (p. 387).
 * @date 2024-01-05
 * 
...
void biggies(std::vector<std::string> &words){
    ...
    //  code to partition the list wiht element whose size is greater than sz preceding the rest
    std::string::size_type sz = 5;
    auto endOfMatches = std::partition(words.begin(), words.end(), [sz](std::string &str){ return str.size() > sz ;});
