/************************************************************************************************************************************************************
 * @file Ex_16_22.cpp
 * @brief Exercise 16.22: Revise your TextQuery programs from § 12.3 (p. 484) so that the shared_ptr members use a 
...
TextQuery::TextQuery(std::ifstream &ifile) {
    ...
    text.reset(new std::vector<std::string> , DebugDelete()) ;    // text is allocated memory and initialised with empty vector of strings
    word_map.reset(new std::map<std::string, std::set<index_type>> , DebugDelete());  // word_map is allocated memory and initialised with empty map
