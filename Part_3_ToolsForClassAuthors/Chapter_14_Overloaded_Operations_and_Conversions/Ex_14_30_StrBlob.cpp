/************************************************************************************************************************************************************
 * @file Ex_14_30.cpp
 * @brief Exercise 14.30: Add dereference and arrow operators to your StrBlobPtr class and to the ConstStrBlobPtr class that
 * you defined in exercise 12.22 from § 12.1.6 (p. 476). Note that the operators in constStrBlobPtr must return const 
 * references because the data member in constStrBlobPtr points to a const vector.
 * @date 2024-01-31
 * 
...
class StrBlobPtr{
    ...
    friend bool operator!=(const StrBlobPtr &lhs, const StrBlobPtr &rhs);
    friend bool operator<(const StrBlobPtr &lhs, const StrBlobPtr &rhs);
    friend StrBlobPtr operator+(const int i, StrBlobPtr lhs);
    friend StrBlobPtr operator-(const int i, StrBlobPtr lhs);
    public :
        StrBlobPtr() : curr(0) {}
        ...
        StrBlobPtr operator--(int);
        StrBlobPtr operator+(const int i);
        StrBlobPtr operator-(const int i);
        std::string & operator*();
        std::string * operator->();
    private : 
...
std::string & StrBlobPtr::operator*(){
    auto p = check(curr, "out of bounds");
    return (*p)[curr];
}
...
StrBlobPtr StrBlobPtr::operator-(const int i){
    check(curr - i, "out of bounds");
    curr -= i;

    return *this;
}

StrBlobPtr operator-(const int i, StrBlobPtr lhs){
    lhs.check(lhs.curr - i, "out of bounds");
    lhs.curr -= i;
